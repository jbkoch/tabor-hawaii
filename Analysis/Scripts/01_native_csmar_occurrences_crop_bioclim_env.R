...
df <- read.table("nat_range/nat_csmar_convhull.csv",
                 header=TRUE, 
                 sep=",",
                 row.names=NULL)
...
dir.create("csmar_env")

#set working directory to new folder
setwd("/home/kochj/hilo_koch_lts/biomod_inR/Tabor-thesis/csmar_env/")

# create directory for the bioclimatic variables
dir.create("bioclim")

#save env conex hull polygon in env folder
writeOGR(env, "csmar_env", layer="csmar_env", driver="ESRI Shapefile")
...
setwd("/home/kochj/hilo_koch_lts/biomod_inR/Tabor-thesis/csmar_env/bioclim/")
